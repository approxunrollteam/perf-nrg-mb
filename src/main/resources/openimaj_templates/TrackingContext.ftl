...
public class TrackingContext {
	...
	void _computeKernels(float sigma, ConvolutionKernel gauss, ConvolutionKernel gaussderiv) {
		...
		/* Compute kernels, and automatically determine widths */
		{
			...
			<#if org_openimaj_video_tracking_klt_TrackingContext__computeKernels_366??>
			${org_openimaj_video_tracking_klt_TrackingContext__computeKernels_366}
			<#else>
            for (i = -hw ; i <= hw ; i++)  {
		...
		int MAX_KERNEL_WIDTH = ConvolutionKernel.MAX_KERNEL_WIDTH;
		/* Shift if width less than MAX_KERNEL_WIDTH */
		<#if org_openimaj_video_tracking_klt_TrackingContext__computeKernels_383??>
		${org_openimaj_video_tracking_klt_TrackingContext__computeKernels_383}
		<#else>
        for (i = 0 ; i < gauss.width ; i++)
        	gauss.data[i] = gauss.data[i+(ConvolutionKernel.MAX_KERNEL_WIDTH-gauss.width)/2];
		</#if>

		<#if org_openimaj_video_tracking_klt_TrackingContext__computeKernels_385??>
		${org_openimaj_video_tracking_klt_TrackingContext__computeKernels_385}
		<#else>
