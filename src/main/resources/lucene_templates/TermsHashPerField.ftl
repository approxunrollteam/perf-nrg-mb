...
import static org.apache.lucene.util.ByteBlockPool.FIRST_LEVEL_SIZE;
...
abstract class TermsHashPerField implements Comparable<TermsHashPerField> {
    ...
    * this.sortedTermIDs to the results */
    ...
    if (ByteBlockPool.BYTE_BLOCK_SIZE - bytePool.byteUpto < numPostingInt* FIRST_LEVEL_SIZE) {
    bytePool.nextBuffer();
    }
    ...
    *  API. */
    ...
    postingsArray.intStarts[termID] = intUptoStart + intPool.intOffset;
  <#if org_apache_lucene_index_TermsHashPerField_add_170??>
      ${org_apache_lucene_index_TermsHashPerField_add_170}
  <#else>
    for(int i=0;i<streamCount;i++) {
    final int upto = bytePool.newSlice(FIRST_LEVEL_SIZE);
    intUptos[intUptoStart+i] = upto + bytePool.byteOffset;
